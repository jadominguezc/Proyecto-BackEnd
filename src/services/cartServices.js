...
const removeProductFromCart = async (cartId, productId) => {
  ...
  const purchaseCart = async (cartId, userEmail) => {
    ...
    if (totalAmount > 0) {
        ...
        // Verificación adicional para asegurar que el ticket fue creado correctamente
        if (ticket && ticket._id) {
            cart.products = missingProducts.length ? cart.products.filter(item => missingProducts.includes(item.product._id)) : [];
            await cartRepository.saveCart(cart);
...
module.exports = { createOrGetCart, updateCart, purchaseCart, clearCart, removeProductFromCart,getCartById };
